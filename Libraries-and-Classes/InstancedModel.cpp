...
InstancedMesh::InstancedMesh(std::vector<InstancedMeshVertex> verts, std::vector<unsigned int> inds, std::vector<InstancedMeshTexture> texs)
{
	vertices = verts;
	indices = inds;
	textures = texs;
...
void InstancedMesh::Init(std::vector<InstancedMeshVertex> verts, std::vector<unsigned int> inds, std::vector<InstancedMeshTexture> texs)
{
	vertices = verts;
	indices = inds;
	textures = texs;
...
void InstancedMesh::Draw(Shader & shader)
{
	...
	glDrawElementsInstanced(GL_TRIANGLES,(GLsizei)indices.size(), GL_UNSIGNED_INT, (void *)0, 10000);
	//check_gl_error();
...
bool InstancedMesh::InitFromSceneMesh(const aiMesh * inputmesh)
{
	return false;
}

glm::mat4x4 InstancedMesh::GetModelMatrix()
{
...
void InstancedMesh::SetupMesh()
{
	...
	// and add the data
	unsigned int size = (unsigned int)vertices.size() * sizeof(InstancedMeshVertex);
	glBufferData(GL_ARRAY_BUFFER, (GLsizei)vertices.size() * sizeof(InstancedMeshVertex), &vertices[0], GL_STATIC_DRAW);
	check_gl_error();
	...
	glVertexAttribPointer(0, 3, GL_FLOAT, GL_FALSE, sizeof(InstancedMeshVertex), (void*)0);
	check_gl_error();
	...
	glVertexAttribPointer(1, 3, GL_FLOAT, GL_FALSE, sizeof(InstancedMeshVertex), (void*)offsetof(InstancedMeshVertex, Normal));
	check_gl_error();
	...
	glVertexAttribPointer(2, 2, GL_FLOAT, GL_FALSE, sizeof(InstancedMeshVertex), (void*)offsetof(InstancedMeshVertex, UV0));
	check_gl_error();
	...
	check_gl_error();
	// setup the attrib pointer to start at correct offset stride is still size off the whole struct
	glVertexAttribPointer(3, 2, GL_FLOAT, GL_FALSE, sizeof(InstancedMeshVertex), (void*)offsetof(InstancedMeshVertex, UV1));
	check_gl_error();
	...
	check_gl_error();
	// setup the attrib pointer to start at correct offset stride is still size off the whole struct
	glVertexAttribPointer(4, 4, GL_FLOAT, GL_FALSE, sizeof(InstancedMeshVertex), (void*)offsetof(InstancedMeshVertex, Colour));
	check_gl_error();
	...
	check_gl_error();
	// setup the attrib pointer to start at correct offset stride is still size off the whole struct
	glVertexAttribPointer(5, 3, GL_FLOAT, GL_FALSE, sizeof(InstancedMeshVertex), (void*)offsetof(InstancedMeshVertex, Tangent));
	check_gl_error();
	...
	check_gl_error();
	// setup the attrib pointer to start at correct offset stride is still size off the whole struct
	glVertexAttribPointer(6, 3, GL_FLOAT, GL_FALSE, sizeof(InstancedMeshVertex), (void*)offsetof(InstancedMeshVertex, Bitangent));
	check_gl_error();

	
}

void InstancedMesh::SetupInstances()
{
	float positionX = -15.0f;
	float positionZ = -15.0f;
	int flowerAmount = 0;
	glm::vec3 offsets[10000];

	//setting the positions of each instance for flower
	for (int x = 0; x < 100; x++)
	{
		for (int z = 0; z < 100; z++)
		{
			offsets[flowerAmount++] = glm::vec3(positionX, 0, positionZ);
			positionZ += 0.3f;
		}
		positionZ = -15.0f;
		positionX += 0.3f;
...
void InstancedModel::LoadModelFromFile(std::string filename)
{
...
void InstancedModel::DebugWriteMesh(aiMesh * mesh, const aiScene * scene)
{
...
{
	vector<InstancedMeshVertex> vertices;
	vector<unsigned int> indices;
	vector<InstancedMeshTexture> textures;
	...
	if (mesh->mMaterialIndex >= 0)
	{
		aiMaterial *material = scene->mMaterials[mesh->mMaterialIndex];
		// diffuse (aka albedo)
		std::vector<InstancedMeshTexture> diffuseMaps = loadMaterialTextures(material,aiTextureType_DIFFUSE, "texture_diffuse");
		textures.insert(textures.end(), diffuseMaps.begin(), diffuseMaps.end());
		// emissive maps (i.e. object glow)
		std::vector<InstancedMeshTexture> emissiveMaps = loadMaterialTextures(material, aiTextureType_EMISSIVE, "texture_emissive");
		textures.insert(textures.end(), emissiveMaps.begin(), emissiveMaps.end());
		// specular color or intensity??
		std::vector<InstancedMeshTexture> specularMaps = loadMaterialTextures(material,aiTextureType_SPECULAR, "texture_specular");
		textures.insert(textures.end(), specularMaps.begin(), specularMaps.end());
		// use ambient to mean AO maps
		std::vector<InstancedMeshTexture> aoMaps = loadMaterialTextures(material, aiTextureType_AMBIENT, "texture_ao");
		textures.insert(textures.end(), aoMaps.begin(), aoMaps.end());
		// normal maps!
		std::vector<InstancedMeshTexture> normalMaps = loadMaterialTextures(material, aiTextureType_NORMALS, "texture_normal");
		textures.insert(textures.end(), normalMaps.begin(), normalMaps.end());
		// bump maps loaded as normals
		std::vector<InstancedMeshTexture> bnormalMaps = loadMaterialTextures(material, aiTextureType_HEIGHT, "texture_normal");
...
std::vector<InstancedMeshTexture> InstancedModel::loadMaterialTextures(aiMaterial * mat, aiTextureType type, std::string typeName)
{
...
void InstancedModel::Render(glm::mat4x4 viewmat, glm::mat4x4 projmat, Shader & shader)
{
...
void InstancedModel::SetPosition(glm::vec3 newpos)
{
	for (unsigned int i = 0; i < meshes.size(); i++)
	{
...
void InstancedModel::SetRotation(glm::quat newrotation)
{
	for (unsigned int i = 0; i < meshes.size(); i++)
	{
...
void InstancedModel::SetScale(glm::vec3 newscale)
{
	for (unsigned int i = 0; i < meshes.size(); i++)
	{
...
void InstancedModel::SetInstances(int instances)
{
	for (unsigned int i = 0; i < meshes.size(); i++)
	{
...
InstancedMesh * InstancedModel::GetMesh(unsigned int index)
{
